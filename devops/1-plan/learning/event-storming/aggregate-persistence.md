...
討論有關impedance-mismatch, 並實作domain model與data model間的轉換

實作Repository pattern抽象化持久化機制，使domain model與service不需相依資料庫相關操作
...
使用value object reference type做為domain model的屬性時，ef可能會發生nullable type exception
    1. 實例化無賦值的value object，使用該作法可降低發生null reference exception
    2. 使用optional types
...
完成以上動作，重構ClassifiedAd's event handle的賦值動作。

---

在使用關聯式資料庫處理aggregate persistence，需要處理domain model 與data model之間的[[impedance-mismatch]]

- 多數開發者會將domain model與data model完全分開，且可以使domain model不受persistence的影響而得到更多靈活性，但相對的會增加處理persistence的成本，在domain model 與data model需要手動進行轉換。在對於大型系統來說，此種方法可能會是首選，因為可以透過調整data model來滿足資料庫的性能需求。

- ORM技術也不斷的改進，優化資料庫性能，提供更多的設定方式，使開發人員可以更加容易的處理impedance mismatch
...
[impedance-mismatch]: impedance-mismatch.md "impedance-mismatch"
